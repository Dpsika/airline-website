...
class GridBuilder extends InjectedModel
{
     ...
     */
    protected function getSortFieldIndex()
    {
        $section = $this->shortcodeModel->section;
        $sortByFieldName = $section->sort_by;
        $availableColumns = $section->getAvailableColumns();

        $keyIndex = array_search($sortByFieldName, array_keys($availableColumns), true);
    ...
    protected function getGridColumns(): array
    {
        ...
        foreach (array_keys($section->getEnabledColumns()) as $attribute) {
            $columnProps = [
